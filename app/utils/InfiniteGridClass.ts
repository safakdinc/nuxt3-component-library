 ...
 * 
 * const grid = new InfiniteGridClass(containerElement, cardDataArray, {
 *   gridCols: 3,
 *   gridRows: 3,
 ...
 * containerElement.addEventListener('tileClicked', (event) => {
 *   console.log('Clicked card:', event.detail.cardData);
 * });
 * 
 * // Clean up when done
 * grid.dispose();
...
export class InfiniteGridClass {
    ...
    this.tileGroupsData.forEach((groupData, groupIndex) => {
      ...
      for (let row = 0; row < this.GRID_ROWS; row++) {
        for (let col = 0; col < this.GRID_COLS; col++) {
          ...
          const backgroundMaterial = this.createBackgroundMaterial(groupIndex, row * this.GRID_COLS + col);
          ...
          const foregroundMaterial = this.createForegroundMaterial(groupIndex, row * this.GRID_COLS + col);
  ...
  private getCardForegroundTexture(groupIndex: number, tileIndex: number): THREE.CanvasTexture | null {
  ...
  private getCardBackgroundTexture(groupIndex: number, tileIndex: number): THREE.CanvasTexture | null {
    if (this.cardTextures.length === 0) return null;
    const textureIndex = this.getCardTextureIndex(groupIndex, tileIndex);
  ...
  private fadeInBackground(mesh: THREE.Mesh<THREE.PlaneGeometry, THREE.ShaderMaterial>): void {
    if (mesh.material instanceof THREE.ShaderMaterial && mesh.material.uniforms && mesh.material.uniforms.uOpacity) {
  ...
  public dispose(): void {
    ...
    console.log('InfiniteGridClass disposed.');
  }
}
