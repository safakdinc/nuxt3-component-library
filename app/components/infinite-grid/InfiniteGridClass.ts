...
import { startAllVideos } from "./createTexture";
...
export class InfiniteGridClass implements EventHandlerHost, DisposableHost, GridManagerHost {
  ...
  public async init(): Promise<void> {
    ...
    // Set up user interaction to enable video playback
  ...
  /**
   * Sets up user interaction handlers to enable video playback
   *
   * Modern browsers require user interaction before allowing video autoplay.
   * This method adds event listeners for the first user interaction to start videos.
   */
  private setupVideoInteraction(): void {
    const startVideoOnInteraction = () => {
      startAllVideos();
      // Remove listeners after first interaction
      this.container.removeEventListener("click", startVideoOnInteraction);
      this.container.removeEventListener("touchstart", startVideoOnInteraction);
      document.removeEventListener("keydown", startVideoOnInteraction);
    };

    // Listen for various user interactions
    this.container.addEventListener("click", startVideoOnInteraction);
    this.container.addEventListener("touchstart", startVideoOnInteraction);
  ...
  private render(): void {
    ...
    // Update video textures for smooth video playback
    this.gridManager.updateVideoTextures();
