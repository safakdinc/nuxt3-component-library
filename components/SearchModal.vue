<template>
  <div
    ...
    <div class="w-full flex flex-col gap-[12px]">
      <div v-for="group in filteredNavigation" :key="group.path || group.title" class="flex flex-col gap-[12px] line pb-3 px-4">
        <div v-if="group.title" class="font-semibold">
          <NuxtLink v-if="group.path && group.page !== false" :to="group.path" class="group-title-link">
            {{ group.title }}
          </NuxtLink>
          <span v-else>{{ group.title }}</span>
        </div>

        <div v-if="group.children && group.children.length > 0" class="flex flex-col gap-[12px] link-group">
          <NuxtLink class="link flex gap-2 items-center" :to="item.path" v-for="item in group.children" :key="item.path">
            <i class="fa-regular fa-file-lines text-[var(--text-gray)]"></i>
            <div class="text-sm">{{ item.title }}</div>
          </NuxtLink>
        </div>
      </div>
      <div v-if="!filteredNavigation || filteredNavigation.length === 0" class="px-4 py-2 text-gray-500">No results found.</div>
    </div>
  </div>
</template>
...
const filterNavigation = (items, query) => {
  ...
  return items
    .map(item => {
      ...
      const itemMatches = item.title?.toLowerCase().includes(query) || item.path?.toLowerCase().includes(query);
