<template>
  <div ref="threeContainer" class="three-container">
    <div class="box absolute top-0 left-0 hidden">
      <div class="w-[52px] h-[52px] bg-red-500 rounded-lg"></div>
    </div>
  </div>
</template>
...
const svgPaths = [
  { direction: "left", path: "M 0, 0 C 274, 0, 578, 69, 842, 186" },
  {
    direction: "right",
    path: "M 0, 0 C 157, 81, 409, 347, 522, 603",
  },
  { direction: "right", path: "M 913, 0 C 1014, 268, 791, 453, 454, 585" },
  { direction: "left", path: "M 506, 0 C 284, 37, 290, 523, 584, 688" },
]; // First part of sine wave
const svgWidth = 100;
const svgHeight = 50;
...
function createInfoBox() {
  ...
  const html = document.querySelector(".box").innerHTML;
  boxElement.innerHTML = html;
}

function calculateOverflow(svgElement, bbox) {
  ...
  if (startX < viewBoxX) {
    startOverflow.x = viewBoxX - startX;
  }
  if (startY < viewBoxY) {
    startOverflow.y = viewBoxY - startY;
  }

  let overflow = bbox.x + bbox.width - (viewBoxX + viewBoxWidth);
  console.log(startOverflow);
  return startX;
}
...
function placeElements() {
  ...
  svgPaths.forEach((svgPath, index) => {
    ...
    const overflow = calculateOverflow(svgElement, bbox);
    console.log(bbox.width - lastElementInfo.oveflow);

    if (index != 0) {
      cssObject.position.set(
        lastElementInfo.right.x +
          bbox.width / 2 -
          lastElementInfo.oveflow -
          bbox.width,
        lastElementInfo.right.y - bbox.height / 2,
        0
      );
    }

    scene.add(cssObject);
    lastElementInfo.oveflow = overflow;
    lastElementInfo.right.x = cssObject.position.x + bbox.width / 2;
    lastElementInfo.right.y = cssObject.position.y - bbox.height / 2;
    lastElementInfo.left.x = cssObject.position.x - bbox.width / 2;
    lastElementInfo.left.y = cssObject.position.y - bbox.height / 2;
  });
}
