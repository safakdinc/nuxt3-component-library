    ...
    <div ref="parallaxContainer" class="w-full flex flex-col gap-[32px] parallax-container">
      ...
      >
        <div
          ...
          :key="itemIndex"
          class="max-w-[480px] max-h-[384px] w-[40vw] aspect-[5/3.5] flex-shrink-0 rounded-lg
            bg-white image-shadow relative group transition-all hover:translate-y-[-15px]"
        >
          <NuxtLink
            v-if="item.link"
            :to="item.link"
          ...
          ></NuxtLink>
          <div
            class="w-full h-full absolute top-0 left-0 z-10 opacity-0 transition-all
              group-hover:opacity-100 flex items-end p-[16px] pointer-events-none"
          ...
          <img
            ...
            :width="800"
            :height="500"
            class="w-full h-full object-cover object-top rounded-lg image"
          />
        </div>
      </div>
    </div>
