<template>
  <div ref="containerRef" class="h-full flex" :class="props.class">
    <div class="">
      <div
        ref="dotRef"
        class="ml-[27px] h-4 w-4 rounded-full border border-neutral-200 shadow-sm flex items-center
          justify-center"
      >
        <div
          ref="innerDotRef"
...
onMounted(() => {
  ...
  scrollerElement.addEventListener("scroll", () => {
    ...
    gsap.to(gradientY2, {
      value:
        scrollY +
        scrollerElement.offsetHeight * (scrollY / svgHeight.value) +
        100 * (scrollY / svgHeight.value),
      duration: 0.6,
      ease: "power2.out",
    });
  });
